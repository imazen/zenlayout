...
mod c_gravity {
    ...
    fn c_gravity_30_70_resolves_to_percentage_0_3_0_7() {
        ...
        let crop = ideal.layout.source_crop.expect("crop mode should produce source_crop");
        ...
        assert_eq!(
            crop.x, 150,
            "x offset should be 30% of 500px slack = 150"
        );
    ...
    fn c_gravity_0_0_crops_from_top_left() {
        ...
        let ideal = plan_layout(
            "w=200&h=200&mode=crop&scale=both&c.gravity=0,0",
            1000,
            800,
        );
    ...
    fn c_gravity_with_mode_crop_positions_correctly() {
        ...
        let ideal_left = plan_layout(
            "w=400&h=400&mode=crop&scale=both&c.gravity=0,50",
            1000,
            500,
        );
    ...
    fn c_gravity_affects_pad_placement() {
        ...
        let ideal_top = plan_layout(
            "w=800&h=600&mode=pad&scale=both&c.gravity=50,0",
            800,
            400,
        );
        ...
        let ideal_bottom = plan_layout(
            "w=800&h=600&mode=pad&scale=both&c.gravity=50,100",
            800,
            400,
        );
        assert_eq!(
        ...
        let ideal_center = plan_layout(
            "w=800&h=600&mode=pad&scale=both&c.gravity=50,50",
            800,
            400,
        );
        assert_eq!(
    ...
    fn c_gravity_50_50_matches_default_center() {
        let ideal_default = plan_layout(
            "w=400&h=400&mode=crop&scale=both",
            1000,
            500,
        );
        ...
        assert_eq!(
            ideal_default.layout.source_crop,
            ideal_50_50.layout.source_crop,
...
mod focus_riapi {
    ...
    /// Document that the `focus` RIAPI key is NOT currently parsed.
    /// This test is ignored because the feature is not yet implemented.
    /// When `focus` parsing is added, remove #[ignore] and update assertions.
    #[test]
    #[ignore = "focus RIAPI key is not yet parsed — gap documented here"]
    fn focus_key_not_yet_parsed() {
        let result = riapi::parse("w=200&h=200&mode=crop&focus=20,50,60,90");
        // Currently `focus` is not recognized, so it produces a warning.
        // When implemented, this should parse into a FocusRect or similar.
        let has_focus_warning = result.warnings.iter().any(|w| matches!(
            w,
            riapi::ParseWarning::KeyNotRecognized { key, .. } if key == "focus"
        ));
        // This assert will FAIL once `focus` is actually parsed (which is the goal):
        assert!(
            !has_focus_warning,
            "focus key should be recognized once implemented"
        );
    }

    #[test]
    fn focus_key_currently_unrecognized() {
        // Verify current behavior: `focus` key produces KeyNotRecognized warning.
        let result = riapi::parse("w=200&h=200&mode=crop&focus=20,50,60,90");
        let has_focus_warning = result.warnings.iter().any(|w| matches!(
            w,
            riapi::ParseWarning::KeyNotRecognized { key, .. } if key == "focus"
        ...
        assert!(
            has_focus_warning,
            "focus key should currently produce KeyNotRecognized warning"
        );
    }
}
